...
import { PostTags, RegistrationObject, Comment } from "../interfaces/interface";
...
// act appropriately
export const handlePostLike = async (setUserLikePost: React.Dispatch<React.SetStateAction<boolean>>, userLikePost: boolean, user_id: string, post_id: string, currentUser: firebase.User) => {
    if (userLikePost) {
        await firebase
            .database()
            .ref('Postsv2')
...
export const onPostComment = async (setPostCommentLoading: React.Dispatch<React.SetStateAction<boolean>>, currentUserInfo: RegistrationObject, post_id: string, username: string, comment: Comment, currentUserToken: string) => {
    if (!currentUserInfo) {
